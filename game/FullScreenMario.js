...
var FullScreenMario = (function(GameStartr) {
    ...
    */
    ...
    /**
     * Location reporting to the ML component Function called on the player every upkeep.
     * 
     * @param {EightBittr} EightBitter
     * @return {Number}   The distance the player has made from the start.
     */
    function reportDistance(EightBitter) {
        var player = EightBitter.player;

        var mapScrollLeft = EightBitter.PixelDrawer.MapScreener.left;

        var curDistance = Math.round(mapScrollLeft + player.left);
        
        if (player.distance != curDistance) {
            player.distance = curDistance;
